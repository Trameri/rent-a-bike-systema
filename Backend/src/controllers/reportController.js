...
export async function summary(req,res){
  try {
    ...
    const rows = await Contract.find(filter).select('totals createdAt location items status').populate('location').lean();
  ...
  total = filteredRows.reduce((s, r) => {
    ...
    } else if (r.totals?.grandTotal) {
      contractTotal = r.totals.grandTotal;
    } else {
...
export async function detailedStats(req, res) {
  ...
  contracts.forEach(contract => {
    ...
    contract.items?.forEach(item => {
      ...
      if(item.insurance) {
        stats.insuranceRevenue += item.insuranceFlat || 0;
      }
    });
