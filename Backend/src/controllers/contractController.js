...
export async function create(req,res){
  ...
  const row = await Contract.create({
    ...
    modificationHistory: [{
      ...
      details: { itemsCount: populated.length, status: status || 'in-use' }
    }]
  });
  res.json(row);
}
...
export async function close(req,res){
  ...
  row.finalPrice = finalPrice || (subtotal + insurance);
  row.finalAmount = finalPrice || (subtotal + insurance); // Campo prioritario per le statistiche
  row.closureNotes = closureNotes || '';
  row.totals = { subtotal, insurance, grandTotal: subtotal + insurance };
...
export async function completePayment(req, res) {
  try {
    ...
    try {
      ...
      // Calcola revenue assicurazioni
      let insuranceRevenue = 0;
      let insuranceCount = 0;
      contract.items.forEach(item => {
        if (item.insurance && item.insuranceFlat) {
          insuranceRevenue += item.insuranceFlat;
          insuranceCount += 1;
        }
      });
      if (contract.insuranceFlat) {
        insuranceRevenue += contract.insuranceFlat;
        insuranceCount += 1;
      }
